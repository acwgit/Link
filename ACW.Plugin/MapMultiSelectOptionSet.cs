    ...
    public class MapMultiSelectOptionSet : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            try
            {
                ...
                string[] sourceAttributes = GetSourceAttributes(executionContext, tracer);
                string[] targetAttributes = GetTargetAttributes(executionContext, tracer);
                EntityReference sourceEntityReference = GetSourceEntityReference(executionContext, service);
                EntityReference targetEntityReference = GetTargetEntityReference(executionContext, service);
                Entity targetEntity = BuildTargetEntity(sourceEntityReference, targetEntityReference, sourceAttributes, targetAttributes, tracer, service, executionContext);

                if (targetEntity != null)
                {
                    service.Update(targetEntity);
                    tracer.Trace("Target entity record updated correctly.");

                }
                else
                    tracer.Trace("Target entity record was NOT updated. ");
